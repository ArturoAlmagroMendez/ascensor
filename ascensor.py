...
class Ascensor:
    ...
    def subir_pisos(self, piso_actual,piso_destino):
        for i in self.pisos:
            ...
            time.sleep(2)
            print(f"{self.estado_actual}... actualmente se encuentra en el piso {piso_actual}")
            if piso_actual == piso_destino:
                self.estado_actual = "parado"
                print(f"Ha llegado a su destino,piso {piso_destino}, el ascensor se ha {self.estado_actual}")
                break
            piso_actual += 1
        #self.piso_actual = piso_actual
